...
{
	IOLog("VoodooSDHCI ::: an SDHCI driver for Ricoh, TI, and JMicron SD Host Controllers ::: rev 20140708\n");
...
bool VoodooSDHC::start ( IOService * provider )
{
	...
	if ((timerActivity = IOTimerEventSource::timerEventSource(this, timerActivityHandler)) == NULL) {
		IOLog("VoodooSDHCI: failed to create a timer event source\n");
		return false;
	}
	if (workLoop->addEventSource(timerActivity) != kIOReturnSuccess) {
		IOLog("VoodooSDHCI: failed ot add TES to work loop\n");
		return false;
	}
...
void VoodooSDHC::stop(IOService *provider)
{
	...
	if (timerActivity != NULL) {
		timerActivity->disable();
		timerActivity->cancelTimeout();
		getWorkLoop()->removeEventSource(timerActivity);
		timerActivity->release();
		timerActivity = NULL;
	}
...
bool VoodooSDHC::willTerminate( IOService * provider, IOOptionBits options )
{
#ifdef __DEBUG__
...
bool VoodooSDHC::cardInit(UInt8 slot)
{
	...
	if(this->PCIRegP[slot]->PresentState & ComInhibitCMD) {
        ...
        //IOLog("VoodooSDHCI: HostControllerVer: 0x%04X\n", this->PCIRegP[slot]->HostControllerVer);
 ...
 */
void VoodooSDHC::SDCommand(UInt8 slot, UInt8 command, UInt16 response,
								UInt32 arg) {
	if (command != 0) {
...
void VoodooSDHC::parseCSD(UInt8 slot) {
	switch ((UInt8)((this->PCIRegP[slot]->Response[3] & 0xC00000) >> 22)) {
		case 0: // version 1
		{
			...
			UInt64 large_to_small = (1 << (blLen)) / 512;
			maxBlock = (cSize+1) * large_to_small * (1 << (cSizeMult+2)) - 1;
            //IOLog("VoodooSDHCI: maxBlock = %llu, version 1\n", maxBlock);
		}

			break;
		case 1: // version 2
		{
			//UInt32 units = (UInt16)(PCIRegP[slot]->Response[1] >> 24);
            ...
            UInt8 rsp[16];
            int i = 0;
            UInt32 str = PCIRegP[slot]->Response[3];
            rsp[i++] = (str & 0xFF0000) >> 16;
            rsp[i++] = (str & 0xFF00) >> 8;
            rsp[i++] = (str & 0xFF);
            
            str = PCIRegP[slot]->Response[2];
            rsp[i++] = (str & 0xFF000000) >> 24;
            rsp[i++] = (str & 0xFF0000) >> 16;
            rsp[i++] = (str & 0xFF00) >> 8;
            rsp[i++] = (str & 0xFF);
            
            str = PCIRegP[slot]->Response[1];
            rsp[i++] = (str & 0xFF000000) >> 24;
            rsp[i++] = (str & 0xFF0000) >> 16;
            rsp[i++] = (str & 0xFF00) >> 8;
            rsp[i++] = (str & 0xFF);
            
            str = PCIRegP[slot]->Response[0];
            rsp[i++] = (str & 0xFF000000) >> 24;
            rsp[i++] = (str & 0xFF0000) >> 16;
            rsp[i++] = (str & 0xFF00) >> 8;
            rsp[i++] = (str & 0xFF);
            
            str = PCIRegP[slot]->Response[3];
            rsp[i++] = (str & 0xFF000000) >> 24;
            
            UInt32 C_SIZE = rsp[9];
            C_SIZE |= rsp[8] << 8;
            C_SIZE |= (rsp[7] & 0x3f) << 16;
            maxBlock = (C_SIZE + 1) * 1024;
            
            //IOLog("VoodooSDHCI: maxBlock = %llu, C_SIZE = %d, version 2\n", maxBlock, C_SIZE);
            
            //IOLog("VoodooSDHCI: SD card CSD = ");
            //for (int i=0; i<16; ++i) IOLog("%02x", rsp[i]);
            //IOLog("\n");
		}
			break;
		default:
...
IOReturn VoodooSDHC::sdma_access(IOMemoryDescriptor *buffer,
					UInt32 block, UInt32 nblks, UInt32 offset, bool read) {
	IOReturn ret = kIOReturnError;
	AbsoluteTime deadline;
    
#ifdef __DEBUG__
    IOLog("VoodooSDHCI sdma_access:  block = %d, nblks = %d\n", block, nblks);
#endif /* __DEBUG__ */
#ifndef NO_RESET_WAR	
	// Reset card before every operation.  The Linux driver
	// does this for this host controller. Not sure why.
	...
	SDCommand(0,
		...
		SDCR18, isHighCapacity ? block : block * 512);
...
IOReturn VoodooSDHC::doAsyncReadWrite(IOMemoryDescriptor *buffer,
	...
	if (buffer->getDirection() == kIODirectionIn) {
		...
		while (n) {
			if (USE_SDMA) {
				int i;
                int b = MIN(8192 /* should fit in sdma buff */, n);
				for (i = 0; i < SDMA_RETRY_COUNT; i++)
                {
					if ((ret = sdma_access(buffer, blk, b, blk - block, true)) != kIOReturnTimeout)
						break;
                }
				if (i != 0 && i < SDMA_RETRY_COUNT)
					IOLog("VoodooSDHCI: read retry succeeded, tries = %d\n", i);
                else if (i >= SDMA_RETRY_COUNT)
                    IOLog("VoodooSDHCI: read retry failed\n");
                n -= b;
                blk += b;
		...
		while (n) {
			if (USE_SDMA) {
				...
				for (i = 0; i < SDMA_RETRY_COUNT; i++)
                {
					if ((ret = sdma_access(buffer, blk, b, blk - block, false)) != kIOReturnTimeout)
						break;
                }
				if (i != 0 && i < SDMA_RETRY_COUNT)
					IOLog("VoodooSDHCI: write retry succeeded, tries = %d\n", i);
                else if (i >= SDMA_RETRY_COUNT)
                    IOLog("VoodooSDHCI: write retry failed\n");
                n -= b;
                blk += b;
			} 
			else if ((nblks > 1) && USE_MULTIBLOCK) {
...
{
	IOLockLock(sdmaCond);
	IOLockWakeup(sdmaCond, sdmaCond, false);
	IOLockUnlock(sdmaCond);
}
...
void VoodooSDHC::handleActivityTimer()
{
	setUserSleepDisabled(false);
}
...
void VoodooSDHC::timerActivityHandler(OSObject *owner, IOTimerEventSource *sender)
{
	VoodooSDHC *self = static_cast<VoodooSDHC*>(owner);
	self->handleActivityTimer();
}

void VoodooSDHC::setUserSleepDisabled(bool disable)
{
    if (disable)
        timerActivity->setTimeoutMS(15000);
...
#ifdef __DEBUG__
    IOLog("%s[%p]::%s(%d)\n", getName(), this, __FUNCTION__, disable ? 1 : 0);
#endif
