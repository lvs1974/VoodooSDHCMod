...
private:
    ...
    IOTimerEventSource	*timerActivity;
	...
	void			SDCommand( UInt8 slot, UInt8 command, UInt16 response, UInt32 arg);
...
//	IOReturn		doDiscard(UInt64 block, UInt64 nblks); /* 10.6.0 */
...
#endif
	IOReturn		sdma_access(IOMemoryDescriptor *buffer, UInt32 block, UInt32 nblks, UInt32 offset, bool read);
    ...
    static void timerActivityHandler(OSObject *owner, IOTimerEventSource *sender);
};
